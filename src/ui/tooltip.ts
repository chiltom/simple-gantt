 ...
 */
export function showTooltip<T extends Item>(
  item: T,
  event: MouseEvent,
  config: GanttConfig<T>,
  svgRect: DOMRect,
): void {
  if (!tooltipElement || !config.showTooltip) return;
  ...
  if (config.customTooltipRenderer) {
    ...
    const fieldsToShow: (keyof T)[] = config.tooltipFields || ["start", "end"];
    fieldsToShow.forEach((fieldKey) => {
      const key: keyof T = fieldKey as keyof T;
          ...
          content += `
            <div class="tooltip-section">
              ...
              <ul class="tooltip-task-list">
                ${value.map((subItem: string) => `<li>${subItem}</li>`).join("")}
              </ul>
            </div>
          `;
        }
  ...
  let top: number = clientY + 15; // Offset from cursor
  let left: number = clientX + 15;
