...
export function getPriorityColor(
  priority: number,
  configuredColors?: { [priorityLevel: string]: string },
): string {
  const mainPriorityLevel: string = Math.floor(priority).toString();

  if (configuredColors && configuredColors[mainPriorityLevel]) {
    return configuredColors[mainPriorityLevel];
  }
  if (defaultPalette[mainPriorityLevel as keyof typeof defaultPalette]) {
    return defaultPalette[mainPriorityLevel as keyof typeof defaultPalette];
  }
...
export function lightenDarkenColor(color: string, percent: number): string {
  ...
  const newColor: string = `${r.toString(16).padStart(2, "0")}${g.toString(16).padStart(2, "0")}${b.toString(16).padStart(2, "0")}`;
...
export function createSvgGradientDefs(configuredColors?: {
  ...
  const defs: SVGDefsElement = document.createElementNS(
    "http://www.w3.org/2000/svg",
    "defs",
  );

  const prioritiesToDefine: string[] = Object.keys(defaultPalette).filter(
    (p) => p !== "default",
  );

  prioritiesToDefine.forEach((priorityKey) => {
    const baseColor: string = getPriorityColor(
      Number(priorityKey),
      configuredColors,
    );
    const lighterColor: string = lightenDarkenColor(baseColor, 20); // 20% lighter for the gradient end
    ...
    const stop1: SVGStopElement = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "stop",
    );
    ...
    const stop2: SVGStopElement = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "stop",
    );
